...
function submitForm () {
    var isAllValid = inputValidation('rUserName', 'The supplied username is too short, try again using 5 or more characters', function (valueString) {return valueString.length >= 5;});
    isAllValid =  inputValidation('rPassword', 'The password needs at least 6 characters', function (valueString) {return valueString.length >= 6;}) && isAllValid;
    isAllValid =  inputValidation('rPassword2', 'Passwords have to be the same', function(valueString) {return valueString === document.getElementById('password').value;}) && isAllValid;
    isAllValid = inputValidation('rPhoneNumber', 'Phone number must be shorter than 15 digits', function (valueString) {return valueString.length < 15;}) && isAllValid;
    isAllValid = inputValidation('rCountry', 'You can only sign-up from Israel or from USA', function (valueString) {return (valueString === "United States" || valueString === "Israel");}) && isAllValid;
    ...
    if (isAllValid) {   
        ...
        document.getElementById('welcome').appendChild(greeting);
    }    
}
